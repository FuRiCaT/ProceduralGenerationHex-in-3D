...
func generate():
	if !grass_enable and !rock_enable and !water_enable and !snow_enable:
		pass
	else:
			...
			for col in range(cols):
				...
				if not loaded_cells.has(cells):
					...
					loaded_cells[cells] = instance
						...
						#a = randi_range(1, 5)
						#if a == 1 or a == 5:
							#var b = preload("res://node_3d.tscn").instantiate()
							#b.global_position = Vector3(x, y, z)
							#add_child(instance)
